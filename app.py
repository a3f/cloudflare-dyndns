...
def main():
    ...
    try:
        ...
        if ipv4 is not None and not a_record:
            return jsonify({'status': 'error', 'message': f'A record for {record_zone_concat} does not exist.'}), 404

        if ipv6 is not None and not aaaa_record:
            return jsonify({'status': 'error', 'message': f'AAAA record for {record_zone_concat} does not exist.'}), 404
...
def healthz():
    ...
    try:
        ...
        return jsonify({'status': 'error', 'message': e.args[1]}), 500
